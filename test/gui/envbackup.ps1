...
if ($errorcode -eq 0 -And -Not [string]::IsNullOrEmpty($Personal)) {
    $retval = backup_directory -keyname "Personal" -newdir $Personal -copied $CopyFiles -removed $RemoveOld -regsetted $RegSetted;
    if ($retval -lt 0) {
...
if ($errorcode -eq 0 -And -Not [string]::IsNullOrEmpty($Desktop)) {
    $retval = backup_directory -keyname "Desktop" -newdir $Desktop -copied $CopyFiles -removed $RemoveOld -regsetted $RegSetted;
    if ($retval -lt 0) {
...
if ($errorcode -eq 0 -And -Not [string]::IsNullOrEmpty($Favorites)) {
    $retval = backup_directory -keyname "Favorites" -newdir $Favorites -copied $CopyFiles -removed $RemoveOld -regsetted $RegSetted;
    if ($retval -lt 0) {
...
if ($errorcode -eq 0 -And -Not [string]::IsNullOrEmpty($MyMusic)) {
    $retval = backup_directory -keyname "My Music" -newdir $MyMusic -copied $CopyFiles -removed $RemoveOld -regsetted $RegSetted;
    if ($retval -lt 0) {
...
if ($errorcode -eq 0 -And -Not [string]::IsNullOrEmpty($MyPic)) {
    $retval = backup_directory -keyname "My Pictures" -newdir $MyPic -copied $CopyFiles -removed $RemoveOld -regsetted $RegSetted;
    if ($retval -lt 0) {
...
if ($errorcode -eq 0 -And -Not [string]::IsNullOrEmpty($MyVideo)) {
    $retval = backup_directory -keyname "My Video" -newdir $MyVideo -copied $CopyFiles -removed $RemoveOld -regsetted $RegSetted;
    if ($retval -lt 0) {
...
if ($errorcode -eq 0 -And -Not [string]::IsNullOrEmpty($Cookies)) {
    $retval = backup_directory -keyname "Cookies" -newdir $Cookies -copied $CopyFiles -removed $RemoveOld -regsetted $RegSetted;
    if ($retval -lt 0) {
...
if ($errorcode -eq 0 -And -Not [string]::IsNullOrEmpty($Cache)) {
    $retval = backup_directory -keyname "Cache" -newdir $Cache -copied $CopyFiles -removed $RemoveOld -regsetted $RegSetted;
    if ($retval -lt 0) {
...
if ($errorcode -eq 0 -And -Not [string]::IsNullOrEmpty($History)) {
    $retval = backup_directory -keyname "History" -newdir $History -copied $CopyFiles -removed $RemoveOld -regsetted $RegSetted;
    if ($retval -lt 0) {
...
if ($errorcode -eq 0 -And -Not [string]::IsNullOrEmpty($Recent)) {
    $retval = backup_directory -keyname "Recent" -newdir $Recent -copied $CopyFiles -removed $RemoveOld -regsetted $RegSetted;
    if ($retval -lt 0) {
...
if ($errorcode -eq 0 -And -Not [string]::IsNullOrEmpty($AppData)) {
    $retval = backup_directory -keyname "AppData" -newdir $AppData -copied $CopyFiles -removed $RemoveOld -regsetted $RegSetted;
    if ($retval -lt 0) {
...
if ($errorcode -eq 0 -And -Not [string]::IsNullOrEmpty($Downloads)) {
    $retval = backup_spec -keyname "download" -itemname $DOWNLOAD_ITEM_NAME -newdir $Downloads -copied $CopyFiles -removed $RemoveOld -regsetted $RegSetted;
    if ($retval -lt 0) {
...
if ($errorcode -eq 0 -And -Not [string]::IsNullOrEmpty($SavedGames)) {
    $retval = backup_spec -keyname "savedgames" -itemname $SAVED_GAME_ITEM_NAME -newdir $SavedGames -copied $CopyFiles -removed $RemoveOld -regsetted $RegSetted;
    if ($retval -lt 0) {
...
if ($errorcode -eq 0 -And -Not [string]::IsNullOrEmpty($Contacts)) {
    $retval = backup_spec -keyname "contacts" -itemname $CONTACTS_ITEM_NAME -newdir $Contacts -copied $CopyFiles -removed $RemoveOld -regsetted $RegSetted;
    if ($retval -lt 0) {
...
if ($errorcode -eq 0 -And -Not [string]::IsNullOrEmpty($Searches)) {
    $retval = backup_spec -keyname "search" -itemname $SEARCH_ITEM_NAME -newdir $Searches -copied $CopyFiles -removed $RemoveOld -regsetted $RegSetted;
    if ($retval -lt 0) {
...
if ($errorcode -eq 0 -And -Not [string]::IsNullOrEmpty($Links)) {
    $retval = backup_spec -keyname "link" -itemname $LINK_ITEM_NAME -newdir $Links -copied $CopyFiles -removed $RemoveOld -regsetted $RegSetted;
    if ($retval -lt 0) {
...
if ($errorcode -eq 0 -And -Not [string]::IsNullOrEmpty($TempFile)) {
    $retval = backup_temp -newdir $TempFile -copied $CopyFiles -removed $RemoveOld -regsetted $RegSetted;
    if ($retval -lt 0) {
